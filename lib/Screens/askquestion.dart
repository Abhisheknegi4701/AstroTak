...
class AstroPageState extends State<AstroPage> {
  ...
  String _choose = "";
  double indexing = 0;
  List<String> questionsuggestion = [];
  ...
  Widget build(BuildContext context) {
    return BlocBuilder<Astrobloc, Astrostate>(builder: (context, state) {
      if (state is Astroquestionloading) {
        ...
        List<String> dropdownlist = [];
        for (int i = 0; i < state.questiionlist.length; i++) {
          dropdownlist.add(state.questiionlist[i].name.toString());
        }
                            ...
                            builder: (context, snapshot) {
                              if (snapshot.data == null ||
                                ...
                                for (int i = 0;
                                  ...
                                  if (_choose == snapshot.data![i].name) {
                                    for (int j = 0;
                                        j <
                                            snapshot
                                                .data![i].suggestions!.length;
                                        j++) {
                                      questionsuggestion.add(
                                          snapshot.data![i].suggestions![j]);
                                    }
                                  }
                                }
                                ...
                                return DropdownButton<String>(
                                  ...
                                  onChanged: (String? value) {
                                    setState(() {
                                      _choose = value!;
                                      questionsuggestion.clear();
                                      for (int i = 0;
                                          i < snapshot.data!.length;
                                          i++) {
                                        indexing = snapshot.data![i].price!;
                                        if (_choose == snapshot.data![i].name) {
                                          for (int j = 0;
                                              j <
                                                  snapshot.data![i].suggestions!
                                                      .length;
                                              j++) {
                                            questionsuggestion.add(snapshot
                                                .data![i].suggestions![j]);
                                          }
                                        }
                                      }
                                    });
                      ...
                      child: Column(
                        ...
                        children: [
                          ...
                          ListView.builder(
                              ...
                              shrinkWrap: true,
                              itemCount: questionsuggestion.length,
                              itemBuilder: (context, index) {
                                if (questionsuggestion.isEmpty) {
                                  return const Center(
                                    child: Text(
                                      "No Item Found",
                                    ),
                                      ...
                                      Row(
                                        children: [
                                          const Icon(
                                            Icons.ac_unit,
                                            color:
                                                Color.fromRGBO(252, 148, 44, 1),
                                          ),
                                          Flexible(
                                            child: Text(
                                              questionsuggestion[index],
                                              style: const TextStyle(
                                                  color: Colors.black,
                                                  fontSize: 18),
                                            ),
